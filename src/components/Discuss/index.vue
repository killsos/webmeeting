    ...
    <div>
        <div class="main">
            <div class="list-container">
                ...
                <div v-show="!messages.length" class="empty">
                    <div class="wrapper">
                        <div class="tips">No chats</div>
                    </div>
                </div>
            </div>
            <Editor :text="text" @send:message="onSendMessage" />
        </div>
    </div>
</template>
<script setup lang="ts" name="Discuss">
    ...
    tryOnMounted(() => {
        if (loadType !== LOAD_TYPES.EMPTY) {
            // first page request
            getMessages(100).then((newMessages: any) => {
    ...
    const addItemClass = (index: number) => {
        return messages.value[index] && messages.value[index].isCreator ? 'creator' : ''
    }
